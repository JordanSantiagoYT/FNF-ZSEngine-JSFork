...
class MemoryUtil
{
	public static var isGcEnabled:Bool = true;
	...
	public static function buildGCInfo():String
	{
		...
		result += '\n- HXCPP Dynamic GC Size: ${#if HXCPP_GC_DYNAMIC_SIZE 'Enabled' #else 'Disabled' #end}';
		result += '\n- HXCPP GC Support over 1GB: ${#if HXCPP_GC_BIG_BLOCKS 'Enabled' #else 'Disabled' #end}';
	...
	public static function enable():Void
	{
		#if cpp
		isGcEnabled = true;
		cpp.vm.Gc.enable(isGcEnabled);
		cpp.NativeGc.enable(isGcEnabled);
		#else
